...
extern void hsv2rgb_raw(const struct CHSV &hsv, struct CRGB &rgb);
...
struct CRGB {
  ...
  inline CRGB(uint32_t colorcode) __attribute__((always_inline))
      : red((colorcode >> 16) & 0xFF), green((colorcode >> 8) & 0xFF),
  ...
  inline CRGB &setHSV(uint8_t hue, uint8_t sat, uint8_t val)
      __attribute__((always_inline)) {
    hsv2rgb_raw(CHSV(hue, sat, val), *this);
    return *this;
  }
  ...
  inline CRGB &setHue(uint8_t hue) __attribute__((always_inline)) {
    hsv2rgb_raw(CHSV(hue, 255, 255), *this);
    return *this;
  }
  ...
  inline CRGB &operator+=(const CRGB &rhs) {
    red = addColor(red, rhs.red);
    green = addColor(green, rhs.green);
    blue = addColor(blue, rhs.blue);
    return *this;
  }
  ...
  inline CRGB &addToRGB(uint8_t d) {
    red = addColor(red, d);
    green = addColor(green, d);
    blue = addColor(blue, d);
    return *this;
  }

  /// Subtract one CRGB from another, saturating at 0x00 for each channel
  inline CRGB &operator-=(const CRGB &rhs) {
    red = subtractColor(red, rhs.red);
    green = subtractColor(green, rhs.green);
    blue = subtractColor(blue, rhs.blue);
    return *this;
  }
  ...
  inline CRGB &subtractFromRGB(uint8_t d) {
    red = subtractColor(red, d);
    green = subtractColor(green, d);
    blue = subtractColor(blue, d);
    return *this;
  }
  ...
  inline CRGB &operator*=(uint8_t d) {
    red = multiplyColor(red, d);
    green = multiplyColor(green, d);
    blue = multiplyColor(blue, d);
    return *this;
  }
  ...
  inline explicit operator uint32_t() const {
    return uint32_t{0xff000000} | (uint32_t{red} << 16) |
  ...
  inline CRGB operator-() const {
    CRGB retval;
    retval.red = 255 - red;
    retval.green = 255 - green;
    retval.blue = 255 - blue;
    return retval;
  }
  ...
  /*#if (defined SmartMatrix_h || defined SmartMatrix3_h)
    ...
    inline CRGB lerp8(const CRGB &other, fract8 frac) const {
      ...
      ret.red = lerp8by8(r, other.red, frac);
      ret.green = lerp8by8(g, other.green, frac);
      ret.blue = lerp8by8(b, other.blue, frac);
    ...
    inline CRGB lerp16(const CRGB &other, fract16 frac) const {
      ...
      ret.red = lerp16by16(r << 8, other.red << 8, frac) >> 8;
      ret.green = lerp16by16(g << 8, other.green << 8, frac) >> 8;
      ret.blue = lerp16by16(b << 8, other.blue << 8, frac) >> 8;
...
inline __attribute__((always_inline)) bool operator==(const CRGB &lhs,
                                                      const CRGB &rhs) {
...
inline __attribute__((always_inline)) bool operator==(const CHSV &lhs,
                                                      const CHSV &rhs) {
...
/// Check if the sum of the color channels in one CRGB object is less than
/// another
inline __attribute__((always_inline)) bool operator<(const CRGB &lhs,
                                                     const CRGB &rhs) {
  uint16_t sl, sr;
  sl = lhs.red + lhs.green + lhs.blue;
  sr = rhs.red + rhs.green + rhs.blue;
  return sl < sr;
}

/// Check if the sum of the color channels in one CRGB object is greater than
/// another
inline __attribute__((always_inline)) bool operator>(const CRGB &lhs,
                                                     const CRGB &rhs) {
  uint16_t sl, sr;
...
/// Check if the sum of the color channels in one CRGB object is less than or
/// equal to another
inline __attribute__((always_inline)) bool operator<=(const CRGB &lhs,
                                                      const CRGB &rhs) {
  uint16_t sl, sr;
...
__attribute__((always_inline)) inline CRGB operator*(const CRGB &p1,
                                                     uint8_t d) {
  return CRGB(qmul8(p1.red, d), qmul8(p1.green, d), qmul8(p1.blue, d));
}*/

/// @copydoc CRGB::operator/=
__attribute__((always_inline)) inline CRGB operator/(const CRGB &p1,
                                                     uint8_t d) {
  return CRGB(p1.red / d, p1.green / d, p1.blue / d);
}

/// Combine two CRGB objects, taking the smallest value of each channel
__attribute__((always_inline)) inline CRGB operator&(const CRGB &p1,
                                                     const CRGB &p2) {
  return CRGB(p1.red < p2.red ? p1.red : p2.red,
