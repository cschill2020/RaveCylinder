...
CHSV &nblend(CHSV &existing, const CHSV &overlay, fract8 amountOfOverlay,
  ...
  existing.saturation = scale8(existing.saturation, amountOfKeep) +
                        scale8(overlay.saturation, amountOfOverlay);
  existing.value = scale8(existing.value, amountOfKeep) +
                   scale8(overlay.value, amountOfOverlay);
...
CHSV ColorFromPalette(const CHSVPalette16 &pal, uint8_t index,
  ...
  uint8_t sat1 = entry->saturation;
  uint8_t val1 = entry->value;
  ...
  if (blend) {
    ...
    uint8_t sat2 = entry->saturation;
    uint8_t val2 = entry->value;
