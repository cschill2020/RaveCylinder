...
typedef struct PlaylistEntry {
  ...
  uint16_t tr;    // Duration of the transition TO this entry (in tenths of
               // seconds)
} ple;

byte playlistRepeat =
    1; // how many times to repeat the playlist (0 = infinitely)
byte playlistEndPreset =
    0; // what preset to apply after playlist end (0 = stay on last preset)
byte playlistOptions =
    0; // bit 0: shuffle playlist after each iteration. bits 1-7 TBD

PlaylistEntry *playlistEntries = nullptr;
byte playlistLen; // number of playlist entries
int8_t playlistIndex = -1;
uint16_t playlistEntryDur =
    0; // duration of the current entry in tenths of seconds
...
void shufflePlaylist() {
  uint8_t currentIndex = playlistLen;
  PlaylistEntry temporaryValue;

  // While there remain elements to shuffle...
  while (currentIndex--) {
    // Pick a random element...
    int randomIndex = random8(0, currentIndex);
...
int16_t loadPlaylist(json& playlistObj, byte presetId) {
  ...
  it = 0;
  json& durations = playlistObj["dur"];
  if (durations.is_null()) {
    playlistEntries[0].dur = playlistObj.value("dur", 100); // 10 seconds as fallback
    it = 1;
  } else {
  ...
  it = 0;
  json& tr = playlistObj[F("transition")];
  if (tr.is_null()) {
    playlistEntries[0].tr =
        playlistObj.value("transition", transitionDelay / 100);
    it = 1;
  } else {
  ...
  playlistRepeat = rep;
  if (playlistRepeat > 0)
    playlistRepeat++; // add one extra repetition immediately since it will be
                      // deducted on first start
  playlistEndPreset = playlistObj.value("end", 0);
  // if end preset is 255 restore original preset (if any running) upon playlist
  // end
  if (playlistEndPreset == 255 && currentPreset > 0)
    playlistEndPreset = currentPreset;
  if (playlistEndPreset > 250) playlistEndPreset = 0;
  int objr = playlistObj.value("r", 0);
  shuffle = shuffle || bool(objr);
  if (shuffle)
...
void handlePlaylist() {
  ...
  if (millis() - presetCycledTime > (100 * playlistEntryDur)) {
    ...
    if (!playlistIndex) {
      ...
      if (playlistRepeat > 1)
        playlistRepeat--; // decrease repeat count on each index reset if not an
                          // endless playlist
    ...
    strip().setTransition(fadeTransition ? playlistEntries[playlistIndex].tr * 100
                                       : 0);
...
void serializePlaylist(json* sObj) {
  json& playlist = (*sObj)["playlist"];
  json& ps = playlist["ps"];
  json& dur = playlist["dur"];
  json& transition = playlist["transition"];
  playlist[F("repeat")] = (playlistIndex < 0 && playlistRepeat > 0)
                              ? playlistRepeat - 1
                              : playlistRepeat; // remove added repetition count
                                                // (if not yet running)
  playlist["end"] = playlistEndPreset;
  playlist["r"] = playlistOptions & PL_OPTION_SHUFFLE;
  for (int i = 0; i < playlistLen; i++) {
    ps.emplace_back(playlistEntries[i].preset);
    dur.emplace_back(playlistEntries[i].dur);
    transition.emplace_back(playlistEntries[i].tr);
  }
}
} // namespace ravecylinder
