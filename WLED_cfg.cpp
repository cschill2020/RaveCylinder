...
// #endif
  ...
  std::string name = id["name"].template get<std::string>();
  getStringFromJson(serverDescription, name.c_str(), 33);
...
//   force802_3g = doc[F("wifi")][F("phy")] | force802_3g; // force phy mode g?
  ...
  json& hw_led = hw["led"];
  strip().ablMilliampsMax = hw_led.value("maxpwr", strip().ablMilliampsMax);
  strip().milliampsPerLed = hw_led.value("ledma", strip().milliampsPerLed);
  //Bus::setGlobalAWMode(hw_led[F("rgbwm")] | AW_GLOBAL_DISABLED);
  correctWB = hw_led.value("cct", correctWB);
  //CJSON(cctFromRgb, hw_led[F("cr")]);
  strip().cctBlending = hw_led.value("cb", strip().cctBlending);
  //Bus::setCCTBlend(strip.cctBlending);
  strip().setTargetFps(hw_led.value("fps", 42)); // NOP if 0, default 42 FPS
  //CJSON(useGlobalLedBuffer, hw_led[F("ld")]);

#ifndef WLED_DISABLE_2D
  // 2D Matrix Settings
  json& matrix = hw_led[F("matrix")];
  if (!matrix.is_null()) {
    strip().isMatrix = true;
    strip().panels = hw_led.value("mpc", strip().panels);
    strip().panel.clear();
    json& panels = matrix[F("panels")];
    uint8_t s = 0;
    if (!panels.is_null()) {
      strip().panel.reserve(std::max(
          (size_t)1, std::min((size_t)strip().panels,
                  (size_t)WLED_MAX_PANELS))); // pre-allocate memory for panels
      for (json& pnl : panels) {
        WS2812FX::Panel p;
        bool bottomStart = p.bottomStart;
        p.bottomStart = pnl.value("b", bottomStart);
        bool rightStart = p.rightStart;
        p.rightStart = pnl.value("r", rightStart);
        bool vertical = p.vertical;
        p.vertical = pnl.value("v", vertical);
        bool serpentine = p.serpentine;
        p.serpentine = pnl.value("s", serpentine);
        p.xOffset = pnl.value("x", p.xOffset);
        p.yOffset = pnl.value("y", p.yOffset);
        p.height = pnl.value("h", p.height);
        p.width = pnl.value("w", p.width);
        strip().panel.push_back(p);
        if (++s >= WLED_MAX_PANELS || s >= strip().panels)
          break; // max panels reached
      }
    } else {
      // fallback
      WS2812FX::Panel p;
  ...
  if (fromFS || !ins.is_null()) {
    ...
    for (json& elm : ins) {
    //   if (s >= WLED_MAX_BUSSES + WLED_MIN_VIRTUAL_BUSSES)
    //     break;
    ...
    //   }
      ...
      uint16_t length = elm.value("len", 1);
    //   uint8_t colorOrder = (int)
    //       elm[F("order")]; // contains white channel swap option in upper nibble
    //   uint8_t skipFirst = elm[F("skip")];
    //  uint16_t start = elm.value("start", 0);
    //  if (length == 0 || start + length > MAX_LEDS)
    //    continue; // zero length or we reached max. number of LEDs, just stop
    //  uint8_t ledType = elm.value("type", TYPE_WS2812_RGB);
      //bool reversed = elm["rev"];
    //  bool refresh = elm.value("ref", false);
    //   uint16_t freqkHz =
    //       elm[F("freq")] | 0; // will be in kHz for DotStar and Hz for PWM (not
    //                           // yet implemented fully)
    //  ledType |=
    ...
    //  uint8_t AWmode = elm[F("rgbwm")] | RGBW_MODE_MANUAL_ONLY;
      if (fromFS) {
        ...
        //  if (busses.add(bc) == -1)
        //    break; // finalization will be done in WLED::beginStrip()
      } /*else {
        if (busConfigs[s] != nullptr)
          delete busConfigs[s];
        busConfigs[s] =
            new BusConfig(ledType, pins, start, length, colorOrder, reversed,
                          skipFirst, AWmode, freqkHz, useGlobalLedBuffer);
        busesChanged = true;
      }*/
      s++;
    }
...
//       if (s > WLED_MAX_COLOR_ORDER_MAPPINGS)
//         break;
...
//   bool pull =
//       btn_obj[F("pull")] | (!disablePullUp); // if true, pullup is enabled
//   disablePullUp = !pull;
//   JsonArray hw_btn_ins = btn_obj[F("ins")];
//   if (!hw_btn_ins.isNull()) {
//     for (uint8_t b = 0; b < WLED_MAX_BUTTONS;
//          b++) { // deallocate existing button pins
//       pinManager.deallocatePin(
//           btnPin[b], PinOwner::Button); // does nothing if trying to deallocate
//                                         // a pin with PinOwner != Button
//     }
//     uint8_t s = 0;
...
//         if (((buttonType[s] == BTN_TYPE_ANALOG) ||
//              (buttonType[s] == BTN_TYPE_ANALOG_INVERTED)) &&
//             (digitalPinToAnalogChannel(btnPin[s]) < 0)) {
//           // not an ADC analog pin
//           DEBUG_PRINT(F("PIN ALLOC error: GPIO"));
//           DEBUG_PRINT(btnPin[s]);
//           DEBUG_PRINT(F("for analog button #"));
//           DEBUG_PRINT(s);
...
//           } else {
// #ifdef ESP32
//             pinMode(btnPin[s], buttonType[s] == BTN_TYPE_PUSH_ACT_HIGH
//                                    ? INPUT_PULLDOWN
//                                    : INPUT_PULLUP);
// #else
...
//       if (++s >= WLED_MAX_BUTTONS)
//         break; // max buttons reached
//     }
...
//   if (serialBaud < 96 || serialBaud > 15000)
//     serialBaud = 1152;
...
//       i2c_scl = i2c_sda = -1;
//     } // this will fail if Wire is initialised (Wire.begin() called prior)
//     else
//       Wire.begin();
// #else
...
//       {spi_mosi, true}, {spi_miso, true}, {spi_sclk, true}};
//   if (spi_mosi >= 0 && spi_sclk >= 0 &&
//       pinManager.allocateMultiplePins(spi, 3, PinOwner::HW_SPI)) {
// #ifdef ESP32
//     SPI.begin(spi_sclk, spi_miso,
//               spi_mosi); // SPI global uses VSPI on ESP32 and FSPI on C3, S3
// #else
//     SPI.begin();
// #endif
//   } else {
...
//   // int hw_status_pin = hw[F("status")]["pin"]; // -1
  ...
  json& light = doc[F("light")];
  if (!light.is_null()) {
    briMultiplier = light.value("scale-bri", briMultiplier);
    strip().paletteBlend = light.value("pal-mode", strip().paletteBlend);
    autoSegments = light.value("aseg", autoSegments);
    gammaCorrectVal = light["gc"]["val"];
  ...
  json& light_tr = light["tr"];
  if(!light_tr.is_null()) {
    fadeTransition = light_tr.value("mode", fadeTransition);
    modeBlending = light_tr.value("fx", modeBlending);
    int tdd = light_tr.value("dur", -1);
  if (tdd >= 0)
    transitionDelay = transitionDelayDefault = tdd * 100;
    strip().setTransition(fadeTransition ? transitionDelayDefault : 0);
    strip().paletteFade = light_tr.value("pal", strip().paletteFade);
    randomPaletteChangeTime = light_tr.value("rpc", randomPaletteChangeTime);
  }

  json& light_nl = light["nl"];
  if(!light_nl.is_null()) {
    nightlightMode = light_nl.value("mode", nightlightMode);
  byte prev = nightlightDelayMinsDefault;
    nightlightDelayMinsDefault = light_nl.value("dur", nightlightDelayMinsDefault);
  if (nightlightDelayMinsDefault != prev)
    nightlightDelayMins = nightlightDelayMinsDefault;

  nightlightTargetBri = light_nl.value("tbri", nightlightTargetBri);
  //CJSON(macroNl, light_nl["macro"]);
  }

  json& def = doc["def"];
  if (!def.is_null()) {
    bootPreset = def.value("ps", bootPreset);
    turnOnAtBoot = def.value("on", turnOnAtBoot);
    briS = def.value("bri", briS);
  }
...
//   if (if_sync_send[F("twice")])
//     udpNumRetries = 1; // import setting from 0.13 and earlier
...
//   if (!DMXAddress || DMXAddress > 510)
//     DMXAddress = 1;
//   CJSON(DMXSegmentSpacing, if_live_dmx[F("dss")]);
//   if (DMXSegmentSpacing > 150)
//     DMXSegmentSpacing = 0;
...
//                     MQTT_MAX_TOPIC_LEN + 1); // "wled/test"
...
//   if (pwdCorrect) { // only accept these values from cfg.json if ota is unlocked
//                     // (else from wsec.json)
...
json doc;
bool success = readObjectFromFile(path("cfg.json"), &doc);
if (!success) { 
  return;
}
  // if file does not exist, optionally try reading from EEPROM
                // and then save defaults to FS
  ...
  //     // call readFromConfig() with an empty object so that usermods can
  //     // initialize to defaults prior to saving
  ...
  bool needsSave = deserializeConfig(doc, true);
  //releaseJSONBufferLock();
...
//     return;
  ...
  json doc;
  //json rev = root["rev"];
  //doc.createNestedArray("rev");
  //rev.emplace_back(1); // major settings revision
  //rev.emplace_back(0); // minor settings revision
...
// #endif
  ...
  json& hw_led = hw["led"];
  hw_led[F("total")] = strip().getLengthTotal(); // no longer read, but provided
                                               // for compatibility on downgrade
  ...
  hw_led[F("rgbwm")] =
      Bus::getGlobalAWMode(); // global auto white mode override
  ...
  if (strip().isMatrix) {
    ...
    json& matrix = hw_led["matrix"];
    matrix[F("mpc")] = strip().panels;
    //JsonArray panels = matrix.createNestedArray(F("panels"));
    json& panels = matrix["panels"];
    for (uint8_t i = 0; i < strip().panel.size(); i++) {
      //JsonObject pnl = panels.createNestedObject();
      json pnl = json::object();
      bool panelSettings = strip().panel[i].bottomStart;
      pnl["b"] = panelSettings;
      panelSettings = strip().panel[i].rightStart;
      pnl["r"] = panelSettings;
      panelSettings = strip().panel[i].vertical;
      pnl["v"] = panelSettings;
      panelSettings = strip().panel[i].serpentine;
      pnl["s"] = panelSettings;
      pnl["x"] = strip().panel[i].xOffset;
      pnl["y"] = strip().panel[i].yOffset;
      pnl["h"] = strip().panel[i].height;
      pnl["w"] = strip().panel[i].width;
      panels.emplace_back(pnl);
    }
  }
#endif
  //JsonArray hw_led_ins = hw_led.createNestedArray("ins");
    json& hw_led_ins = hw_led["ins"];
...
//   //hw_status["pin"] = -1;
  ...
  json& light = doc["light"];
  light[F("scale-bri")] = briMultiplier;
  light[F("pal-mode")] = strip().paletteBlend;
  light[F("aseg")] = autoSegments;

  json& light_gc = light["gc"];
  light_gc["bri"] =
      (gammaCorrectBri) ? gammaCorrectVal : 1.0f; // keep compatibility
  light_gc["col"] =
      (gammaCorrectCol) ? gammaCorrectVal : 1.0f; // keep compatibility
...
//   f.close();
  ...
  std::cout<<"config doc: "<<doc<<std::endl;
}
